...
namespace ECE_323_LAB_10
{
    ...
    public partial class Form1 : Form
    {
        ...
        public byte[] inputarr = new byte[510];
        ...
        private void DataReceivedHandler(object sender, SerialDataReceivedEventArgs e) // handler for new data recieved
        {
            ...
            if (sp.BytesToRead >= DatabufferSize) // if bytes to read is equal to data length
            {
                ...
                /*for (int i = 55; i < 255; i++) // convert from byte array to double array
                {
                    ...
                    plot_array[i - 55] = plot_array[i - 55] / 255 * 5; // convert from adc values to Voltages
                }

                for (int i = 310; i < 510; i++) // convert from byte array to double array
                {
                    digi_array[i - 310] = Convert.ToDouble(inputarr[i]); // convert from byte array to double array

                    digi_array[i - 310] = digi_array[i - 310] / 255 * 5; // convert from adc values to Voltages
                }*/

                for (int i = 0; i < 200; i++) // convert from byte array to double array
                {
                ...
                in_avg = Queryable.Average(plot_array.AsQueryable()); // get avg value from array 
                out_avg = Queryable.Average(digi_array.AsQueryable()); // get avg value from array 

                incoming_rigger_value = digi_array[0]; // get trigger value from first element in the array

                this.Invoke(new EventHandler(plot_data)); // invoke new thread to handle data plottting

                // remove DC componets of signal
                for (int i = 0; i < 200; i++)
                {
                ...
                sp.DiscardInBuffer(); // discard data in buffer
        ...
        private void show_input_freq(object sender, EventArgs e) // DFT Algorithm 
        {
            ...
            pout = Array.IndexOf(resultout, oout);
             ...
             textBox7.Text = "Input Freq = " + Math.Round(Convert.ToDouble(samp_freg) / data_length * p, 2) + "Hz"; // display input freq in toxbox 7

            textBox9.Text = "Output Freq = " + Math.Round(Convert.ToDouble(samp_freg) / data_length * pout, 2) + "Hz";

            //textBox7.Text = "Input Freq = " + 1.5 * p;  //Math.Round(Convert.ToInt32(samp_freg) / data_length * p, 2) + "Hz"; // display input freq in toxbox 7

            //textBox9.Text = "Output Freq = " + 1.5 * p;
        ...
        private void plot_data(object sender, EventArgs e) // plots data on mschart
        {
            ...
            if (ADC == true) // if chart is not cleared 
            {
                ...
                textBox6.Text = "P2P = " + (array_max - array_min) + "V"; // Show P2P on textbox6
